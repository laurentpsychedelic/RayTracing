...
template <typename T, typename U>
const Ray<T, U> SphericalSurfacePropagator<T, U>::propagate(const Ray<T, U>& ray, const ISurface3D<T>* _surface) {
    if (ray.direction.z <= 0) // The ray is lost
        return Ray<T, U>(ray.location, ray.direction, 0.0); // 0 intensity
    const SphericalSurface3D<T>* surface = dynamic_cast<const SphericalSurface3D<T>* >(_surface);
    if (!surface)
