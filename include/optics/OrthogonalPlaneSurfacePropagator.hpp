...
template <typename T, typename U>
const Ray<T, U> OrthogonalPlaneSurfacePropagator<T, U>::propagate(const Ray<T, U>& ray, ISurface3D<T>* _surface) {
    if (ray.direction.z <= 0) // The ray is lost
        return Ray<T, U>(ray.location, ray.direction, 0.0); // 0 intensity
    const OrthogonalPlaneSurface3D<T>* surface = dynamic_cast<OrthogonalPlaneSurface3D<T>* >(_surface);
    if (!surface)
